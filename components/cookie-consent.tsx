...
function CookieToggle({
  ...
  return (
    <div className="flex items-start justify-between gap-6 border border-black/5 bg-zinc-50 p-5">
      ...
      <button
        ...
        disabled={disabled}
        onClick={onChange}
        className={`relative inline-flex h-8 w-14 shrink-0 transition-colors ${
          checked ? "bg-primary" : "bg-black/10"
        } ${disabled ? "cursor-not-allowed opacity-70" : ""}`}
      >
        <span
          className={`absolute top-1 h-6 w-6 bg-white shadow-sm transition-transform ${
    ...
    <CookieConsentContext.Provider value={contextValue}>
      ...
      {isMounted && isBannerVisible ? (
        <div className="fixed inset-x-0 bottom-0 z-[70] p-4 sm:p-6">
          <div className="mx-auto w-[min(96vw,1380px)] border border-black/10 bg-white p-6 shadow-[0_20px_60px_rgba(0,0,0,0.18)] sm:p-7">
            <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <button
                  ...
                  className="inline-flex border border-black/10 bg-white px-6 py-3 text-[11px] font-black uppercase tracking-[0.22em] text-navy transition-colors hover:border-primary hover:text-primary"
                >
                  Len nevyhnutné
                </button>
                <button
                  type="button"
                  onClick={acceptAll}
                  className="inline-flex bg-navy px-6 py-3 text-[11px] font-black uppercase tracking-[0.22em] text-white transition-colors hover:bg-primary"
                >
                  Prijať všetko
                </button>
              </div>
      ...
      {isMounted && isModalOpen ? (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 p-4">
          <div className="w-full max-w-3xl bg-white p-6 shadow-[0_24px_80px_rgba(0,0,0,0.22)] sm:p-8">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="inline-flex h-10 w-10 items-center justify-center border border-black/10 text-xl text-navy transition-colors hover:border-primary hover:text-primary"
            ...
            <div className="mt-8 flex flex-col gap-3 border-t border-black/5 pt-6 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <button
                  type="button"
                  onClick={acceptAll}
                  className="inline-flex border border-primary/20 bg-primary/10 px-6 py-3 text-[11px] font-black uppercase tracking-[0.22em] text-primary transition-colors hover:bg-primary hover:text-white"
                >
                  Prijať všetko
                </button>
                <button
